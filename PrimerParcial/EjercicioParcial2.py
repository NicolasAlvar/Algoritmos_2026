# DATOS ARCHIVO SUPER_HEROES_DATA
...
ordenados_nombre = sorted(superheroes, key=lambda h: h["name"])
for h in ordenados_nombre:
    print(f"  {h['name']}")

# 2. Posicion de "The Thing" y "Rocket Raccoon" en la lista ordenada

print("\n--- 2. Posicion de 'The Thing' y 'Rocket Raccoon' (lista ordenada por nombre) ---")
nombres_ordenados = [h["name"] for h in ordenados_nombre]

for objetivo in ["The Thing", "Rocket Raccoon"]:
    if objetivo in nombres_ordenados:
        posicion = nombres_ordenados.index(objetivo) + 1  # base 1
        print(f"  '{objetivo}' esta en la posicion {posicion}")
    else:
...
# 4. Cola (queue) de villanos + filtrar los anteriores a 1980

print("\n--- 4. Cola de villanos y cuales aparecieron antes de 1980 ---")
from collections import deque

cola_villanos = deque([v["name"] for v in villanos])
print(f"  Cola de villanos: {list(cola_villanos)}")

print("\n  Villanos que aparecieron antes de 1980:")
villanos_antes_1980 = [v for v in villanos if v["first_appearance"] < 1980]
for v in villanos_antes_1980:
    print(f"    {v['name']} ({v['first_appearance']})")

# 5. Superheroes que comienzan con Bl, G, My, W

print("\n--- 5. Superheroes cuyo nombre comienza con Bl, G, My o W ---")
prefijos = ("Bl", "G", "My", "W")
filtrados_prefijo = [h for h in superheroes if h["name"].startswith(prefijos)]
for h in filtrados_prefijo:
    print(f"  {h['name']}")

# 6. Listado ordenado por NOMBRE REAL ascendente

print("\n--- 6. Personajes ordenados por nombre real (ascendente) ---")
# Excluimos los que tienen real_name = None para no romper el sort
...
print("\n--- 7. Superheroes ordenados por fecha de aparicion ---")
ordenados_fecha = sorted(superheroes, key=lambda h: h["first_appearance"])
...
print("\n--- 9. Personajes cuya bio incluye 'time-traveling' o 'suit' ---")
bio_filtrados = [
    h for h in superheroes
    if "time-traveling" in h["short_bio"].lower() or "suit" in h["short_bio"].lower()
]
for h in bio_filtrados:
    print(f"  {h['name']}")

# 10. Eliminar a Electro y Baron Zemo y mostrar su info

print("\n--- 10. Eliminar a Electro y Baron Zemo y mostrar su informacion ---")
objetivos_eliminar = ["Electro", "Baron Zemo"]
for nombre in objetivos_eliminar:
    encontrado = None
    for h in superheroes:
        if h["name"] == nombre:
            encontrado = h
    ...
    if encontrado:
        ...
        print(f"\n  '{nombre}' encontrado y eliminado. Su informacion:")
